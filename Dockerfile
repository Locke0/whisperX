...
RUN --mount=type=cache,id=pip-$TARGETARCH$TARGETVARIANT,sharing=locked,target=/root/.cache/pip \
    pip install -U --timeout=120 --force-reinstall pip setuptools wheel && \
    pip install -U --timeout=360 --extra-index-url https://download.pytorch.org/whl/cu121 \
...
ENTRYPOINT ["dumb-init", "--", "uvicorn", "whisperx.server:app", "--host", "0.0.0.0", "--port", "8000"]

ARG VERSION
ARG RELEASE
LABEL name="whisperx-server" \
      maintainer="yangyuewang123@gmail.com" \
      version=${VERSION} \
      release=${RELEASE} 
